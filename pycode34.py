...
class AdvancedAlgorithm:
    ...
    def optimize_placement(self, table_data):
        placements = []
        ...
        for data in table_data:
            ...
            for i in range(ilosc):
                ...
                # Sprawdź, czy nowy obszar koliduje z istniejącymi obszarami
                while self.check_collision(placements, x_position, y_position, b, h):
                    x_position = random.randint(0, self.green_area_width - b)
                    y_position = random.randint(0, self.green_area_height - h)
    ...
    def check_collision(self, existing_placements, x, y, width, height):
        for placement in existing_placements:
            x_existing, y_existing = placement
            if (
                x < x_existing + width and
                x + width > x_existing and
                y < y_existing + height and
                y + height > y_existing
            ):
                return True
        return False
...
class Ui_MainWindow:
    ...
    def update_red_areas(self):
        ...
        placements = self.advanced_algorithm.optimize_placement(table_data)
        ...
        for i, data in enumerate(table_data):
            ...
            for j in range(ilosc):
                x_position, y_position = placements.pop(0)
                red_area = self.canvas.create_rectangle(x_position, y_position, x_position + b, y_position + h, fill="red", outline="black")
